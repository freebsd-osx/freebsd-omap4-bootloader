...
struct emif_reg_struct {
	...
	uint32_t padding9[6];
	uint32_t emif_ddr_phy_status[21];
	uint32_t padding10[27];
...
struct dmm_lisa_map_regs {
	uint32_t dmm_lisa_map_0;
	uint32_t dmm_lisa_map_1;
	uint32_t dmm_lisa_map_2;
	uint32_t dmm_lisa_map_3;
	uint8_t is_ma_present;
};
...
struct lpddr2_addressing {
	uint8_t	num_banks;
	uint8_t	t_REFI_us_x10;
	uint8_t	row_sz[2]; /* One entry each for x32 and x16 */
	uint8_t	col_sz[2]; /* One entry each for x32 and x16 */
};
...
void emif_get_reg_dump(uint32_t emif_nr, const struct emif_regs **regs);
void emif_get_dmm_regs(const struct dmm_lisa_map_regs **dmm_lisa_regs);

void do_ext_phy_settings(uint32_t base, const struct emif_regs *regs);
void get_lpddr2_mr_regs(const struct lpddr2_mr_regs **regs);

extern uint32_t *const T_num;
extern uint32_t *const T_den;

void config_data_eye_leveling_samples(uint32_t emif_base);
uint32_t emif_sdram_type(void);
const struct read_write_regs *get_bug_regs(uint32_t *iterations);
#endif
