...
create_image() {
	...
	DISK_MD=`sudo -E mdconfig -a -t vnode -f $IMG -x 63 -y 255`
}
...
freebsd_boot() {
	...
	eval $buildenv make -C ${FREEBSD}/sys/boot/arm/omap4 \
				UBLDR_LOADADDR=${UBLDR_LOADADDR} all 2>&1 | tee boot_build.log
#	eval sudo -E $buildenv make -C ${FREEBSD}/sys/boot/arm/uboot \
